...
public struct TermsOfService: Decodable {
    static let classificationSummaries: [TermsOfService.Classification: PrivacyPractice.Summary] = [
        .a: .good,
        .b: .mixed,
        .c: .poor,
        .d: .poor
    ...
    var summary: PrivacyPractice.Summary {
    ...
    static func load() -> [String: TermsOfService]? {
        guard let url = Bundle(for: Dependencies.self).url(forResource: "tosdr", withExtension: "json") else { return nil }
        guard let data = try? Data(contentsOf: url) else { return nil }
        return try? JSONDecoder().decode([String: TermsOfService].self, from: data)
    }
